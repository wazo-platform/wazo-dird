...
class TestPhoneView(BaseDirdIntegrationTest):
    ...
    def test_given_invalid_offset_then_lookup_return_400(self):
        result = self.get_lookup_cisco_result(self.profile, VALID_UUID, term='A', token=VALID_TOKEN, offset=-1)
        assert_that(result.status_code, equal_to((400)))

    def test_given_invalid_limit_then_lookup_return_400(self):
        result = self.get_lookup_cisco_result(self.profile, VALID_UUID, term='A', token=VALID_TOKEN, limit=-1)
        assert_that(result.status_code, equal_to((400)))
    ...
    def test_that_menu_return_input_url_when_no_proxy(self):
        result = self.get_menu_cisco(self.profile, VALID_UUID, token=VALID_TOKEN)
    ...
    def test_that_menu_return_input_url_when_profile_name_menu(self):
        profile = 'menu'
        result = self.get_menu_cisco(profile, VALID_UUID, token=VALID_TOKEN)
    ...
    def test_that_input_return_lookup_url_when_no_proxy(self):
        result = self.get_input_cisco(self.profile, VALID_UUID, token=VALID_TOKEN)
    ...
    def test_that_input_return_lookup_url_when_profile_name_input(self):
        profile = 'input'
        result = self.get_input_cisco(profile, VALID_UUID, token=VALID_TOKEN)
    ...
    def test_that_lookup_return_400_when_no_term(self):
        result = self.get_lookup_cisco_result(self.profile, VALID_UUID,  token=VALID_TOKEN)

        assert_that(result.status_code, equal_to(400))

    def test_that_lookup_return_404_when_unknown_profile(self):
        result = self.get_lookup_cisco_result('quiproquo', VALID_UUID, term='alice', token=VALID_TOKEN)

        assert_that(result.status_code, equal_to(404))

    def test_aastra_input_route(self):
        result = self.get_input_aastra_result(self.profile, VALID_UUID, token=VALID_TOKEN)
        assert_that(result.status_code, equal_to((200)))

    def test_aastra_lookup_route(self):
        result = self.get_lookup_aastra_result(self.profile, VALID_UUID, term='Alice', token=VALID_TOKEN)
        assert_that(result.status_code, equal_to((200)))

    def test_cisco_input_route(self):
        result = self.get_input_cisco_result(self.profile, VALID_UUID, token=VALID_TOKEN)
        assert_that(result.status_code, equal_to((200)))

    def test_cisco_menu_route(self):
        result = self.get_menu_cisco_result(self.profile, VALID_UUID, token=VALID_TOKEN)
        assert_that(result.status_code, equal_to((200)))

    def test_cisco_lookup_route(self):
        result = self.get_lookup_cisco_result(self.profile, VALID_UUID, term='Alice', token=VALID_TOKEN)
        assert_that(result.status_code, equal_to((200)))

    def test_polycom_input_route(self):
        result = self.get_input_polycom_result(self.profile, VALID_UUID, token=VALID_TOKEN)
        assert_that(result.status_code, equal_to((200)))

    def test_polycom_lookup_route(self):
        result = self.get_lookup_polycom_result(self.profile, VALID_UUID, term='Alice', token=VALID_TOKEN)
        assert_that(result.status_code, equal_to((200)))

    def test_snom_input_route(self):
        result = self.get_input_snom_result(self.profile, VALID_UUID, token=VALID_TOKEN)
        assert_that(result.status_code, equal_to((200)))

    def test_snom_lookup_route(self):
        result = self.get_lookup_snom_result(self.profile, VALID_UUID, term='Alice', token=VALID_TOKEN)
        assert_that(result.status_code, equal_to((200)))

    def test_thomson_lookup_route(self):
        result = self.get_lookup_thomson_result(self.profile, VALID_UUID, term='Alice', token=VALID_TOKEN)
        assert_that(result.status_code, equal_to((200)))

    def test_yealink_lookup_route(self):
        result = self.get_lookup_yealink_result(self.profile, VALID_UUID, term='Alice', token=VALID_TOKEN)
        assert_that(result.status_code, equal_to((200)))

    def test_gigaset_lookup_route(self):
        result = self.get_lookup_gigaset_result(self.profile, VALID_UUID, term='Alice', token=VALID_TOKEN)
        assert_that(result.status_code, equal_to((200)))

    def test_htek_lookup_route(self):
        result = self.get_lookup_htek_result(self.profile, VALID_UUID, term='Alice', token=VALID_TOKEN)
        assert_that(result.status_code, equal_to((200)))
