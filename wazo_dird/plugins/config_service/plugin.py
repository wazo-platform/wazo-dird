...
class ConfigServicePlugin(BaseServicePlugin):
    ...
    def load(self, dependencies):
        ...
        return Service(config, bus, controller, database.TenantCRUD(Session), confd_client)
...
class Service:
    ...
    def _sync_tenants(self):
        ...
        for tenant in tenants:
            try:
                localization = self._confd_client.localization.get(tenant_uuid=tenant['uuid'])
            except Exception:
                continue
            else:
                self._tenant_crud.create_or_edit(tenant['uuid'], {'country': localization['country']})
