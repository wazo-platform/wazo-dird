...
class UnresolvedFavorite(TypedDict):
    contact_id: str
    user_uuid: str
...
class SourceReport(TypedDict):
    ...
    deduplicated: int
    unresolved: list[UnresolvedFavorite]
    error: str | None
...
class FavoriteMigrationService:
    ...
    def migrate(self) -> MigrationReport:
        ...
        for source in self._source_service.list_(BACKEND, None):
            ...
            report['unresolved'] += len(source_report['unresolved'])
    ...
    def _migrate_source(self, source: SourceInfo) -> SourceReport | None:
        ...
        for contact_id, user_uuid in pending:
            ...
            if not new_contact_id:
                logger.warning(
                    'source %s: favorite %s of user %s matches no confd user, keeping it',
                    source['name'],
                    contact_id,
                    user_uuid,
                )
